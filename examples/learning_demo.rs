...
fn main() {
    ...
    println!(
        "Pool: {} neurons, {} synapses ({:.0}% density)",
        initial_stats.n_neurons,
        initial_stats.n_synapses,
        initial_stats.n_synapses as f64 / (N as f64 * N as f64) * 100.0
    );
    println!(
        "  Exc: {}, Inh: {}",
        initial_stats.n_excitatory, initial_stats.n_inhibitory
    );
    ...
    for epoch in 0..MAX_EPOCHS {
        ...
        if epoch % 20 == 0 || epoch < 5 {
            ...
            println!(
                "Epoch {:4} | out={:3} | total={:3} | syn={:3} | |w|={:.0} | {}",
                epoch,
                output_spikes,
                total_spikes,
                stats.n_synapses,
                stats.mean_weight_magnitude,
                format_thermal(&stats.thermal)
            );
        }
    }
    ...
    println!(
        "  Final output:     {} spikes / {} ticks",
        final_response, PRESENT_TICKS
    );
    ...
    if quarter > 0 {
        ...
        let q4 = responses[quarter * 3..].iter().sum::<u32>() as f64
            / (responses.len() - quarter * 3) as f64;
        println!();
    ...
    println!("Proven:");
    println!(
        "  [{}] STDP eligibility traces mark active pathways",
        if final_stats.mean_eligibility_magnitude > 0.0 {
            "x"
        } else {
            " "
        }
    ...
    println!(
        "  [{}] DA modulation strengthens traced synapses (|w| grew)",
        if final_stats.mean_weight_magnitude > initial_stats.mean_weight_magnitude {
            "x"
        } else {
            " "
        }
    ...
    println!(
        "  [{}] Thermal maturity: synapses progressed past HOT",
        if final_stats.thermal.warm + final_stats.thermal.cool + final_stats.thermal.cold > 0 {
            "x"
        } else {
            " "
        }
    ...
    println!(
        "  [{}] Structural pruning: dead synapses removed",
        if final_stats.n_synapses < initial_stats.n_synapses {
            "x"
        } else {
            " "
        }
    ...
    println!(
        "  [{}] Homeostatic stability: output rate maintained",
...
fn format_thermal(td: &ThermalDistribution) -> String {
    format!(
        "H:{} W:{} C:{} F:{} D:{}",
        td.hot, td.warm, td.cool, td.cold, td.dead
    )
}
