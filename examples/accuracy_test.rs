...
fn main() {
    ...
    let sensory_neurons = pool
        .get_template(sensory_id)
        .unwrap()
        .neuron_indices
        .clone();
    let encoder_neurons = pool
        .get_template(encoder_id)
        .unwrap()
        .neuron_indices
        .clone();
    ...
    println!(
        "  Encoder:  {} neurons (pattern sharpening)",
        encoder_neurons.len()
    );
    println!(
        "  Memory:   {} neurons (attractor storage)",
        memory_neurons.len()
    );
    println!(
        "  Intent:   {} neurons (category decision)",
        intent_neurons.len()
    );
    println!(
        "  Synapses: {} feed-forward + {} recurrent\n",
    ...
    for (word, bytes, intent) in &vocabulary {
        ...
        for &byte in *bytes {
            for tick in 0..8 {
                ...
                total_spikes[0] += sensory_neurons
                    .iter()
                    .filter(|&&i| pool.neurons.spike_out[i])
                    .count() as u32;
                total_spikes[1] += encoder_neurons
                    .iter()
                    .filter(|&&i| pool.neurons.spike_out[i])
                    .count() as u32;
                total_spikes[2] += memory_neurons
                    .iter()
                    .filter(|&&i| pool.neurons.spike_out[i])
                    .count() as u32;
                total_spikes[3] += intent_neurons
                    .iter()
                    .filter(|&&i| pool.neurons.spike_out[i])
                    .count() as u32;
            }
        ...
        let memory_pattern: Vec<u32> = memory_neurons
            .iter()
            .enumerate()
        ...
        println!(
            "  {:5} [{:?}] → spikes: S={:3} E={:3} M={:3} I={:3} | active: {:?}",
            word,
            intent,
            total_spikes[0],
            total_spikes[1],
    ...
    for (expected_word, bytes, expected_intent) in &vocabulary {
        ...
        let resting = pool.config.resting_potential;
        let threshold = resting + 500;
        let query_pattern: Vec<u32> = memory_neurons
            .iter()
            .enumerate()
            .filter(|(_, &i)| pool.neurons.membrane[i] > threshold)
            .map(|(idx, _)| idx as u32)
    ...
    for (novel_word, bytes) in &novel_words {
        ...
        let mut matches: Vec<(&str, usize, Intent)> = learned_patterns
            .iter()
    ...
    for intent in &intents {
        let words_with_intent: Vec<&str> = learned_patterns
            .iter()
            .filter(|(_, _, i)| i == intent)
            .map(|(w, _, _)| *w)
            .collect();

        let patterns: Vec<&Vec<u32>> = learned_patterns
            .iter()
            .filter(|(_, _, i)| i == intent)
            .map(|(_, p, _)| p)
            .collect();
    ...
    println!(
        "  Recall accuracy:     {}/{} ({:.0}%)",
        recall_correct, recall_total, recall_accuracy
    );
    ...
    println!(
        "  Avg Jaccard sim:     {:.3} (lower = more distinct)",
        avg_jaccard
    );
